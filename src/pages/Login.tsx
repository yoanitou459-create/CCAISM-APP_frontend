  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      if (code === 'auth/popup-blocked') {
        setError("Le popup Google a été bloqué par le navigateur.");
      } else if (code === 'auth/unauthorized-domain') {
        setError("Domaine non autorisé pour Google. Ajoutez votre URL Vercel dans Firebase → Domaines autorisés.");
      } else {
        setError('Erreur de connexion Google: ' + message);
      }
    } finally {
      setIsVerifying(false);
    }
        ...
        <div className="auth-form-panel">
          <div className="max-w-sm mx-auto w-full space-y-6">
            <div className="space-y-1">
              ...
              <h1 className="text-3xl font-bold text-cscm-green tracking-tight">Connexion</h1>
              <p className="text-sm text-gray-400 font-normal">Accédez à la plateforme CCAISM</p>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              ...
              <button type="submit" id="login-submit-btn" disabled={isVerifying} className="auth-submit-btn disabled:opacity-50 disabled:cursor-not-allowed">
                {isVerifying ? 'Vérification en cours...' : 'Se connecter'}
              </button>
            </form>

            <div className="flex justify-between items-center text-xs font-medium pt-1">
              <Link to="/signup" className="text-cscm-green hover:underline underline-offset-2">
                Créer un compte
              </Link>
              <Link to="/forgot-password" className="text-gray-500 hover:text-cscm-green transition-colors">
                Mot de passe oublié ?
              </Link>
            </div>
          </div>
